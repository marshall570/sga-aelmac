...
class AssistedController:
    ...
    def insert_assisted(self, a):
        try:
            ...
            root.withdraw()
            messagebox.showinfo(
                'CADASTRADO', 'Assistido CADASTRADO com sucesso!')
            tkinter.Tk().destroy()
    ...
    def select_assisted(self, i):
        try:
            ...
            cursor = conn.cursor()
            rs = cursor.execute(
                f'SELECT * FROM tb_assistidos WHERE Posicao = {i}').fetchone()
            return rs
    ...
    def delete_assisted(self, i):
        try:
            ...
            cursor.execute(
                f'UPDATE tb_assistidos SET Posicao = Posicao - 1 WHERE Posicao > {i}')
            conn.commit()
    ...
    def print_register(self, a, index):
        try:
            ...
            query = cursor.execute(
                f'SELECT * FROM tb_assistidos WHERE Posicao = {index}').fetchone()

            fields = ['SERIAL', 'ID', 'Nome do Assistido', 'Data de Nascimento', 'Telefone (Celular)', 'Telefone (Residencial)', 'Gênero', 'Estado Civil', 'Ocupação', 'Reside Com', 'Endereço', 'Bairro', 'Número', 'Cidade', 'Estado',
                      'Toma sedativos', 'Tratamento médico', 'Dorme bem', 'Vícios', 'Sonhos', 'Trabalho', 'Família', 'Alimentação', 'Info para DEPOE', 'Último Tratamento', 'Frequência - Cursos', 'Encaminhamento', 'Tratamentos', 'Orientação']
            ...
            while i < len(fields):
                if query[i] != 'Não' and query[i] != '':
                    ...
                    pdf.multi_cell(0, 10, query[i].replace(
                        ' --- ', ', '), 1, 'J', 0)
                i += 1
            ...
            if self.count_interviews(a) > 0:
                ...
                fields_interview = [
                    'DATA', 'ENTREVISTADOR', 'TRATAMENTO', 'ENTREVISTA']
                ...
                while i < len(fields_interview):
                    if latest_interview[i] != 'Não' and latest_interview[i] != '':
                        pdf.set_font('helvetica', 'B', size=11)
                        pdf.cell(
                            55, 10, fields_interview[i].upper(), 1, 0, '', 1, '')
                        pdf.set_font('helvetica', size=11)
                        pdf.multi_cell(0, 10, latest_interview[i].replace(
                            ' --- ', ', '), 1, 'J', 0)
                    i += 1
            ...
            root.withdraw()
            messagebox.showinfo(
                'GERAR FICHA', 'Ficha para impressão gerada com sucesso para a pasta <FICHAS_INDIVIDUAIS>!')
            tkinter.Tk().destroy()
    ...
    def gen_custom_csv(self, custom_string, selected_filter):
        try:
            ...
            if platform.system() == 'Linux':
                db_df.to_excel(os.path.expanduser('~') + '/Documentos/GERENCIAMENTO_DE_ASSISTIDOS_AELMAC/RELATORIOS_EXCEL/FILTRAGEM_POR_' +
                               selected_filter.upper() + '_' + today + '.xlsx', index=False)
            else:
                db_df.to_excel(os.path.expanduser('~') + '\\Documents\\GERENCIAMENTO_DE_ASSISTIDOS_AELMAC\\RELATORIOS_EXCEL\\FILTRAGEM_POR_' +
                               selected_filter.upper() + '_' + today + '.xlsx', index=False)

            root = tkinter.Tk()
            root.withdraw()
            messagebox.showinfo(
                'SUCESSO', 'Relatório para EXCEL gerado com sucesso!')
            tkinter.Tk().destroy()
    ...
    def export_data(self):
        ...
        if platform.system() == 'Linux':
            shutil.copyfile('sga_database.db', os.path.expanduser(
                '~') + '/Documentos/GERENCIAMENTO_DE_ASSISTIDOS_AELMAC/BACKUPS/BACKUP' + '_' + today + '.db')
        else:
            shutil.copyfile('sga_database.db', os.path.expanduser(
                '~') + '\\Documents\\GERENCIAMENTO_DE_ASSISTIDOS_AELMAC\\BACKUPS\\BACKUP' + '_' + today + '.db')

        root = tkinter.Tk()
        root.withdraw()
        messagebox.showinfo(
            'SUCESSO', 'BACKUP do banco de dados gerado com sucesso!\nSaindo do gerenciador de backups...')
        tkinter.Tk().destroy()
    ...
    def import_data(self, file):
        ...
        root.withdraw()
        messagebox.showinfo(
            'SUCESSO', 'Dados importados e substituidos com sucesso!\nSaindo sistema para aplicar atualizações...')
        tkinter.Tk().destroy()
    ...
    def select_active_user(self):
        try:
            ...
            cursor = conn.cursor()
            rs = cursor.execute(
                'SELECT Nome, Usuario, Categoria FROM tb_usuarios WHERE Status = \'ON\'').fetchone()
            return rs
    ...
    def set_off(self):
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                'UPDATE tb_usuarios SET Status = \'OFF\' WHERE Status = \'ON\'')
            conn.commit()
    ...
    def register_changes(self, name, message):
        try:
            ...
            index = cursor.execute(
                'SELECT COUNT(*) FROM tb_historico').fetchone()[0] + 1
    ...
    def count_interviews(self, a):
        try:
            ...
            cursor = conn.cursor()
            rs = cursor.execute(
                f'SELECT COUNT(*) FROM tb_entrevistas WHERE id_assistido = \'{a.serial}\'').fetchone()[0]
            return rs
    ...
    def insert_interview(self, i, a):
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                f'INSERT INTO tb_entrevistas VALUES (\'{i.code}\', \'{a.serial}\', \'{i.date}\', \'{i.interviewer}\', \'{i.treatment}\', \'{i.interview}\')')
            conn.commit()

            cursor.execute(
                f'UPDATE tb_assistidos SET Ultimo_tratamento = \'{i.treatment}\' WHERE Serial = \'{a.serial}\'')
            conn.commit()

            root = tkinter.Tk()
            root.withdraw()
            messagebox.showinfo(
                'CADASTRADO', 'Entrevista registrada com sucesso!')
            tkinter.Tk().destroy()
    ...
    def select_interview(self, a):
        try:
            ...
            cursor = conn.cursor()
            rs = cursor.execute(
                f'SELECT Data_da_entrevista, Entrevistador, Tratamento, Entrevista FROM tb_entrevistas WHERE id_assistido = \'{a.serial}\'').fetchall()
    ...
    def print_interviews(self, a):
        try:
            ...
            fields_interview = ['DATA', 'ENTREVISTADOR',
            ...
            if platform.system() == 'Linux':
                pdf.output(os.path.expanduser(
                    '~') + '/Documentos/GERENCIAMENTO_DE_ASSISTIDOS_AELMAC/FICHAS_INDIVIDUAIS/Entrevistas_de_' + a.name + '.pdf')
            else:
                pdf.output(os.path.expanduser(
                    '~') + '\\Documents\\GERENCIAMENTO_DE_ASSISTIDOS_AELMAC\\FICHAS_INDIVIDUAIS\\Entrevistas_de_' + a.name + '.pdf')
