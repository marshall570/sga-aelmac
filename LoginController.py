...
class LoginController:
    ...
    def count_user(self, user):
        try:
            ...
            cursor = conn.cursor()
            rs = cursor.execute(
                f'SELECT COUNT(*) FROM tb_usuarios WHERE Usuario = \'{user}\'').fetchone()[0]
            return rs
    ...
    def check_password(self, user, password):
        try:
            ...
            verification = cursor.execute(
                f'SELECT Senha FROM tb_usuarios WHERE Usuario = \'{user}\'').fetchone()[0]
    ...
    def select_user(self, user):
        try:
            ...
            cursor = conn.cursor()
            rs = cursor.execute(
                f'SELECT Usuario, Senha, Categoria FROM tb_usuarios WHERE Usuario = \'{user}\'').fetchone()
            return rs
    ...
    def set_on(self, user):
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                f'UPDATE tb_usuarios SET Status = \'ON\' WHERE Usuario = \'{user}\'')
            conn.commit()
